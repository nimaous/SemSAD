...
class Get_Score(object):
    ...
    def matrix_preparation(self, max_size, data_loader):        
        ...
        x_t1_total = torch.stack(x_t1_lst).view(-1, c, width, height)
        x_t2_total = torch.stack(x_t2_lst).view(-1, c, width, height)
        x_o_total = torch.stack(x_o_lst).view(-1, c, width, height)
        x_n_total = torch.stack(x_n_lst).view(-1, c, width, height)
        h_total = torch.stack(h_lst).view(-1, h_dim)
    ...
    def score_function(self,):
        ...
        self.score_ind_train = self.s_net(torch.cat((best_match_ind_train, self.x_ind_train), dim=1)).sum(-1)
        self.score_ind = self.s_net(torch.cat((best_match_ind, self.x_ind), dim=1)).sum(-1)
        self.score_ood = self.s_net(torch.cat((best_match_ood, self.x_ood), dim=1)).sum(-1)
        
        l = len(self.x_ind_train)
        rnd_idx = list(range(l))
        random.shuffle(rnd_idx)
        self.x_pos_ref = torch.cat((self.x_ind_train[0:l//self.div], self.x_t1[l//self.div:]), dim=0)[rnd_idx] 
        self.x_pos_match = torch.cat((best_match_ind_train[0:l//self.div], self.x_t2[l//self.div:]), dim=0)[rnd_idx]
        
        self.score_ind_train_pos = self.s_net(torch.cat((self.x_pos_ref, self.x_pos_match), dim=1)).sum(-1)
        self.score_ind_train_neg = self.s_net(torch.cat((self.x_pos_ref, self.x_ind_train_neg), dim=1)).sum(-1)
                
        self.calculate_aucroc(self.score_ind, self.score_ood, self.ind_name+'_'+self.ood_name)
        self.dist_plot()
        return self.score_ind_train, self.score_ind, self.score_ood 
    
    def find_best_match(self,): 
        h, x, x_n, self.x_t1, self.x_t2 = self.matrix_preparation(self.max_size1, self.loader)
        h_d, x_d = h[self.max_size2:], x[self.max_size2:]

        self.h_ind_train, self.x_ind_train = h[0:self.max_size2], x[0:self.max_size2] 
        self.h_ind, self.x_ind, _, _, _ = self.matrix_preparation(self.max_size2, self.ind_loader)
        self.h_ood, self.x_ood, _, _, _ = self.matrix_preparation(self.max_size2, self.ood_loader)
    ...
    def dist_plot(self,):
        ...
        if self.train_hist:
            ...
            plt.hist(self.score_ind_train_pos.cpu(), bins=num_bins, alpha=0.8) 
            plt.hist(self.score_ind_train_neg.cpu(), bins=num_bins, alpha=0.8)                    
            plt.xlabel("OOD score", fontsize=10)
            plt.ylabel("Frequency", fontsize=10)
            plt.legend([self.ind_name+'-train', self.ind_name+'-test', self.ood_name+'-test', self.ind_name+'-train_pos', 
