...
class UnifRep(EfficientRandomGen):
    ...
    def __call__(self, example):
        example = self.replace_tokens(example)
        if example.text_b:
...
def get_data_stats(examples):
    ...
    for i in range(len(examples)):
        #print(f"example: {i}")
        cur_word_dict = {}
        cur_sent = copy.deepcopy(examples[i][1])
...
def word_level_augment(examples, aug_ops, vocab, data_stats=None):
    """Word level augmentations. Used before augmentation.
        vocab is only required for Uniform Replace 
