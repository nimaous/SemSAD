...
def train(epoch, args, 
    ...
    for itr, ((img_norm, img_t1, img_t2), label) in enumerate(loader): 
        ...
        loader.set_description(
            (
                f' Epoch: {epoch + 1};  NCELoss: {nce_loss.item()}'                
            )
        )
        if vis:
            writer.add_scalar("NCE Loss", nce_loss.item(), 
                              global_step=epoch, walltime=wt) 
            writer.add_scalar("Accuracy", accuracy.item(), 
                              global_step=epoch, walltime=wt) 
    if vis:
        with torch.no_grad(): 
            if epoch % 100 == 0:
                ...
                writer.add_embedding(tag="h1", mat=h_net(img_norm), 
                                     metadata=label_lst, label_img=img_norm, 
                                     global_step=epoch)            
...
if __name__ == '__main__':
    ...
    h_net = resnet18(pretrained=False, 
                     progress=False, 
                     num_classes=args.h_dim)
    h_net = modify_resnet_model(h_net, args, 
                                mode='encoder')    
    print("Number of h net Paramters: ", count_parameters(h_net))
    
    # defining the optimiser 
    h_net_opt = optim.Adam(h_net.parameters(), 
                          lr=args.h_lr, 
    ...
    ds_warped = DataSetWrapper(args.dataset, args.ds_dir, 
                              args.bs, None, args.nw, mode='encoder')
    ...
    for epoch in range(args.epoch):
        train(epoch, args, 
              loader, h_net, h_net_opt)
        torch.save(
             {'model': h_net.module.state_dict(),
              'args': args}, f'checkpoint/{exp_num}_h_net.pt',)
